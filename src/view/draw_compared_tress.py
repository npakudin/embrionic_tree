...
def draw_node(draw, node1, node2, global_params, border_left, border_top, border_right, border_bottom, level,
    ...
    if not left1.is_none() or not left2.is_none():
        ...
        [add_max_dist, add_min_dist] = draw_node(draw, left1, left2, global_params, border_left, border_top, right, item_top, level+1, min_reduced_depth, start_numbers, parent=(center_x, center_y))
    ...
    if right1 is not None or right2 is not None:
        ...
        [add_max_dist, add_min_dist] = draw_node(draw, right1, right2, global_params, left, border_top, border_right, item_top, level+1, min_reduced_depth, start_numbers, parent=(center_x, center_y))
        total_max_distance += add_max_dist
        total_min_distance += add_min_dist
...
def draw_tree(tree1, tree2, global_params, dist, param_a, folder):

    im = Image.new('RGBA', [1500, 600], (255, 255, 255, 255))
    draw = ImageDraw.Draw(im)

    min_reduced_depth = min(tree1.root.reduced_depth, tree2.root.reduced_depth)
    max_reduced_depth = max(tree1.root.reduced_depth, tree2.root.reduced_depth)
    ...
    for i in range(0, 11):
        draw.text((im.size[0] - 20, im.size[1] - (i + 1)*(ITEM_SIZE + ITEM_SPACE)), f"{i + 1}", fill='black')
